...
class ImageNetDataReader(CalibrationDataReader):
    def __init__(
        ...
        self.end_index = (
            len(os.listdir(self.image_folder)) if end_index == 0 else end_index
        )
    ...
    def preprocess_imagenet(
        self, images_folder, height, width, start_index=0, size_limit=0
    ):
        """
        ...
        for image_name in batch_filenames:
            ...
            image_size_list.append(
                np.array([img.size[1], img.size[0]], dtype=np.float32).reshape(1, 2)
            )
    ...
    def get_synset_id(self, image_folder, offset, dataset_size):
        ...
        file = open(
            image_folder + "/devkit/data/ILSVRC2012_validation_ground_truth.txt", "r"
        )
...
class ImageClassificationEvaluator:
    ...
    def predict(self):
        ...
        session = onnxruntime.InferenceSession(
            self.model_path, sess_options=sess_options, providers=self.providers
        )
        ...
        while True:
            ...
            print(f"{(end - start) * 1000} ms/iter")
...
def get_dataset_size(dataset_path, calibration_dataset_size):
    ...
    if calibration_dataset_size == 0:
        logging.warning(
            "No dataset is assigned for calibration. Please use bigger dataset"
        )

    prediction_dataset_size = (
        (total_dataset_size - calibration_dataset_size) // batch_size
    ) * batch_size
    if prediction_dataset_size <= 0:
        logging.warning(
            "No dataset is assigned for evaluation. Please use bigger dataset"
        )
...
if __name__ == "__main__":
    ...
    2. Download ILSVRC2012  dataset and development kit from http://www.image-net.org/challenges/LSVRC/2012/downloads.
    3. Extract  dataset JPEG files to 'ILSVRC2012/val'.
    4. Extract development kit to 'ILSVRC2012/devkit'. Two files in the development kit are used, 'ILSVRC2012__ground_truth.txt' and 'meta.mat'.
    ...
    ilsvrc2012_dataset_path = "/home/ubuntu/dataset_imagenet"
    augmented_model_path = "./augmented_model.onnx"
    batch_size = 10
    calibration_dataset_size = 100  # Size of dataset for calibration
    ...
    [calibration_dataset_size, prediction_dataset_size] = get_dataset_size(
        ilsvrc2012_dataset_path, calibration_dataset_size
    )
    print(f"{calibration_dataset_size=}")
    print(f"{prediction_dataset_size=}")
    calibration_dataset_size = 100
    prediction_dataset_size = 1000

    # Generate INT8 calibration table
    if calibration_table_generation_enable:
        calibrator = create_calibrator(
            new_model_path, [], augmented_model_path=augmented_model_path
        )
        calibrator.set_execution_providers(["CUDAExecutionProvider"])
        data_reader = ImageNetDataReader(
            ilsvrc2012_dataset_path,
            start_index=0,
        ...
        ranges = {}
        for name, _range in calibrator.compute_data().data.items():
            ranges[name] = _range.range_value
        write_calibration_table(ranges)
    ...
    synset_id = data_reader.get_synset_id(
        ilsvrc2012_dataset_path, calibration_dataset_size, prediction_dataset_size
    )  # Generate synset id
    evaluator = ImageClassificationEvaluator(
        new_model_path, synset_id, data_reader, providers=execution_provider
    )
    evaluator.predict()
